...
{ key: 'synopsis', label: '02', short: '扩写梗概', title: '扩成 4 分 30 秒的总梗概', desc: '这一步先做完整故事，再拆成三次作业。' },
...
{ key: 'prompts', label: '05', short: '分段提示词', title: '按 10 秒一段生成 27 组提示词', desc: '每 9 组对应一次 1 分 30 秒作业，剧情、首尾帧、运镜、配乐、台词、剧本都放在这里。' },
...
function canPreviewImageLink(value) {
  return /^https?:\/\//i.test(value) || value.startsWith('blob:') || value.startsWith('/api/uploads/') || value.startsWith('/aimanju-api/uploads/');
}
...
function buildExternalSegmentPrompt({
  ...
  return `你是一名专门给 AI 短剧拆分分镜提示词的导演兼编剧。现在要把一个 4 分 30 秒的 AI 漫剧期末作品，拆成按 10 秒一段的分段提示词。
...
- 总时长：270 秒
- 需要拆分段数：27 段
- 课程作业结构：3.2.1、3.2.2、3.2.3 三次作业，每次 1 分 30 秒，每次 9 个 10 秒视频片段
...
2. 时间段必须连续，且每段 10 秒。
3. 第 1-9 段为 3.2.1 第一阶段，第 10-18 段为 3.2.2 第二阶段，第 19-27 段为 3.2.3 第三阶段。
4. 第 1 段必须写“首帧提示词”和“尾帧提示词”；第 2-27 段不要写“首帧提示词”，只写“首帧来源：承接上一段尾帧，不重新生成首帧图”和“尾帧提示词”。
5. 每段都必须写“作业阶段、标题、时间段、剧情说明、出场角色、运镜提示词、配乐提示词、台词/配音提示词、剧本提示词”。
      ...
      return (
        <div className="ai-manju-step-layout ai-manju-step-layout-prompts">
            ...
            <div className="ai-manju-empty-state">
	                            ...
	                            <p>{'这里会按 10 秒一段解析 27 组提示词，每 9 组对应一次 1 分 30 秒作业。'}</p>
	                        </div>
            }
            ...
            <article key={segment.id} className="ai-manju-segment-card">
                            <div className="ai-manju-panel-head">
                                <div>
                                    <span className="ai-manju-chip">{segment.assignmentPhase || segment.timeRange}</span>
                                    {segment.assignmentPhase && <span className="ai-manju-chip">{segment.timeRange}</span>}
                                    <h3>{segment.title}</h3>
                                </div>
                                <button
                  type="button"
                  ...
                  <article key={`preview-${segment.id}`} className="ai-manju-final-row">
                                            <div className="ai-manju-final-row-main">
                                                <div className="ai-manju-final-row-head">
                                                    <div>
                                                        <span className="ai-manju-chip">{segment.assignmentPhase || segment.timeRange || '分段'}</span>
